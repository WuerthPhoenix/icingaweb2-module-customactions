...
namespace Icinga\Module\Customactions\RestApi;
...
class CustomRestApiClient
{
    ...
    protected function request($method, $url, $body = null, $headers = [])
    {
        ...
        if($statusCode < 400){
            $res = (object) array("status" => "Success", "error" => $statusCode);
        }
        
        if($statusCode >= 500){
            $res = (object) array("status" => "Internal server error", "error" => $statusCode);
        }
